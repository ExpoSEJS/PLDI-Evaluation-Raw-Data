...
function util(origState) {
  ...
  var rtn = {
    delete: function _delete(id, entityType) {
    ...
    replace: function replace(id, entityType, value, data) {
      operation(entityType, function (ops) {
    ...
    iterate: function iterate(entityType, callback) {
...
function reducer(options) {
  ...
  var entityType = options.entityType,
      ...
      afterReduce = options.afterReduce,
      debug = options.debug;

  var verbose = debug === 'verbose';
  var log = (0, _commonUtil.logger)('model-reducer "' + entityType + '"');
  ...
  function update(_ref) {
    ...
    var stateEntities = Object.assign({}, state.entities);
    ...
    if (beforeReduce) {
      ...
      beforeReduce(action, util, { action: action, id: id, entities: entities, value: result });
    ...
    // update the metadata
    var metaDomain = stateEntities._meta[entityType] = Object.assign({}, stateEntities._meta[entityType]);
    ...
    if (afterReduce) {
      ...
      afterReduce(action, context, action, util, { action: action, id: id, entities: entities, value: result });
      state.entities = context.execute();
    }
  ...
  if (verbose) {
    log('loaded\n\tlistening for ' + handlers.map(function (data) {
      return data[0];
    }).join(', '));
  }
  ...
  return function () {
    ...
    for (var i = 0; i < handlers.length; i++) {
      if (handlers[i][0] === type) {
        ...
        var id = (payload.id === false ? NO_ID : payload.id) || result;
...
function updateEntityModels(values, entities, primaryId, primaryEntityType, fetchData) {
  ...
  var _meta = rtn._meta = Object.assign({}, entities._meta);
