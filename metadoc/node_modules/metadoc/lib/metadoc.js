        ...
        if (atxStyleHeadingPattern.test(line)) {
            ...
            if ((match || []).length > 1) {
                ...
                if (endWithWhitespaceattern.test(sectionName)) {
                    ...
                    sectionName = sectionName.replace(endWithSharpPattern, '').replace(endWithWhitespaceattern, '');
                }
                sections.push(sectionName);
                lastLine = '';
            } else {
...
function Metadoc (config) {
    config = config || {};
    this.config = {
        fromPath: config.fromPath || path.join(__dirname, '../src/docs'),
        toPath: config.toPath || path.join(__dirname, '../docs'),
...
Metadoc.prototype.process = function () {
    ...
    this.themePath = path.join(__dirname, themeRoot, cfg.theme);
    ...
    if (existsSync(path.join(this.themePath, 'assets'))) {
        ncp(path.join(this.themePath, 'assets'), path.join(this.toPath, 'assets'), function (){});
    }
};
...
Metadoc.prototype.genHtml = function (data, systems) {
    ...
    if (data.isModule) {
        ...
        title = data.systemName + ' - ' + data.name;
    } else {
        systemId = data.id;
        breadcrumbHtml = tpl.tmpl(breadcrumb, {});
        data.path = systemId;
    ...
    headerHtml = tpl.tmpl(header, {title: title});
    html = tpl.tmpl(page, {body: data.body, header: headerHtml, footer:footer});
    data.html = html;
    return pageHtml;
};
...
Metadoc.prototype.getData = function (rootDirectory, iterate, mdFilter) {
    var metaInfo = this.getMetaInfo(rootDirectory) || {},
        ...
        sidebarOn = metasidebarOn = metaInfo.sidebarOn,
    ...
    if (metasidebarOn !== true && metasidebarOn !== false) {
    ...
    files.forEach(function (f, i) {
        ...
        secs.forEach(function (s, i) {
            ...
            if (sections.indexOf(s) >= 0) {
                console.error('Duplicate Section Name:' + s + ' Founded in file:' + f);
                return;
            } else {
                if (index >= 0) {
                sections[index] = s;
