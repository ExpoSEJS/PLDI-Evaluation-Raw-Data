(function() {
  ...
  Tiny.Util.isFunction = function(functionToCheck) {
    var getType;
    getType = {};
...
(function() {
  Tiny.Store = (function() {
    ...
    Store.prototype.find = function(type, id, data, callback) {
      var i, len, record, ref;
      if (Tiny.Util.isFunction(data)) {
        callback = data;
        data = [];
      }
      if ((this.records[type] != null) === false) {
      ...
      if (this.driver !== null) {
        this.driver.find(type, id, data, (function(_this) {
    ...
    Store.prototype.fetch = function(type, page, data, callback) {
      console.log('fetching type ' + type + 'page ' + page);
      console.log('data:');
      console.log(data);
      if (Tiny.Util.isFunction(data)) {
        callback = data;
      ...
      return this.driver.fetch(type, page, data, (function(_this) {
...
(function() {
  ...
  Tiny.Collection = (function(superClass) {
    ...
    Collection.prototype.modelize = function(type) {
      var i, index, len, record, results;
      results = [];
      for (i = 0, len = this.length; i < len; i++) {
        record = this[i];
        index = this.indexOf(record);
        results.push(this[index] = Tiny.App.instance.store.modelize(type, this.record));
      }
      return results;
    };
...
(function() {
  Tiny.Store.RESTDriver = (function() {
    ...
    RESTDriver.prototype.find = function(type, id, data, callback) {
      var url;
      data = data.join('/');
      url = this.apiURL + type + '/' + id + '/' + data;
      url = url.replace(/\/$/, '');
      console.log(url);
      return $.get(url, callback);
    };

    RESTDriver.prototype.fetch = function(type, page, data, callback) {
      var url;
      data = data.join('/');
      url = this.apiURL + type + '/page/' + page + '/' + data;
      url = url.replace(/\/$/, '');
      console.log(url);
