...
describe('Observable', function() {
    ...
    it('Should be able to write to multiple observable properties on a model object using chaining syntax', function() {
        ...
        assert(model.prop1()).toEqual('A');
        assert(model.prop2()).toEqual('B');
    });
    ...
    it('Should be able to use Function.prototype methods to access/update', function() {
        ...
        assert(instance.apply(null, [])).toBe('B');
    });
    ...
    it('Should notify subscribers about each new value', function () {
        ...
        assert(notifiedValues.length).toEqual(2);
        assert(notifiedValues[0]).toEqual('A');
        assert(notifiedValues[1]).toEqual('B');
    });
    ...
    it('Should notify "beforeChange" subscribers before each new value', function () {
        ...
        assert(notifiedValues.length).toEqual(2);
        assert(notifiedValues[0]).toEqual(undefined);
        assert(notifiedValues[1]).toEqual('A');
    });
    ...
    it('Should ignore writes when the new value is primitive and strictly equals the old value', function() {
        ...
        for (var i = 0; i < 3; i++) {
            instance("A");
            assert(instance()).toEqual("A");
            assert(notifiedValues).toEqual(["A"]);
        }

        instance("B");
        assert(instance()).toEqual("B");
        assert(notifiedValues).toEqual(["A", "B"]);
    });
    ...
    it('Should notify subscribers of a change when an object value is written, even if it is identical to the old value', function() {
        ...
        assert(notifiedValues).toEqual([constantObject]);
    });
    ...
    it('Should ignore writes when the equalityComparer callback states that the values are equal', function() {
        ...
        instance(null);
        assert(notifiedValues.length).toEqual(3);
        ...
        instance(undefined);
        assert(notifiedValues.length).toEqual(4);

        // undefined vs object - change
        instance({ id: 1 });
        assert(notifiedValues.length).toEqual(5);
    });
    ...
    it('Should be possible to replace notifySubscribers with a custom handler', function() {
        ...
        assert(interceptedNotifications.length).toEqual(2);
        assert(interceptedNotifications[0].eventName).toEqual("beforeChange");
        assert(interceptedNotifications[1].eventName).toEqual("None");
