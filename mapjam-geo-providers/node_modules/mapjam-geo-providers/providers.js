...
module.exports = {
    ...
    switch(provider) {
      ...
      case 'mapzen':
        ...
        replyObj.address = {
          ...
          pretty: response.features[0].properties.label,
          address: response.features[0].properties.name,
          city: response.features[0].properties.locality,
          region: response.features[0].properties.region,
          country: response.features[0].properties.country,
          countryCode: response.features[0].properties.country_a,
          postal: response.features[0].properties.postalcode 
        };
        ...
        replyObj.feature = {
          ...
          properties: replyObj.address                  
        };
        break;
      case 'HERE':
        var xmlDoc = libxmljs.parseXml(response);
        var lat = xmlDoc.get('//Latitude').text();
        var lng = xmlDoc.get('//Longitude').text();
  ...
  geocode: function(provider, input, inputOpts, callback) {
    ...
    if (inputOpts) _.assign(requestOpts, inputOpts.request);
    if (inputOpts) _.assign(queryOpts, inputOpts.query);
    switch(provider) {
      case 'google':
        if(!queryOpts.sensor) queryOpts.sensor = false;
        requestOpts.urlBuilder.host = "maps.googleapis.com:443";
      ...
      case 'mapzen':
        ...
        if(typeof input === "string" || input instanceof String) {          
          ...
          queryOpts['layers'] = 'address';
          requestOpts.urlBuilder.pathname = "/v1/reverse";
        } else {
          var err = new Error('You didn\t provide a valid input. Valid inputs are string or array')
          callback(err);
        }      
        break;
      case 'HERE':
        requestOpts.urlBuilder.host = "geocoder.cit.api.here.com:443";
        requestOpts.urlBuilder.pathname = "/6.2/geocode.xml";
        queryOpts.gen = 9;
        queryOpts.app_id = inputOpts.app_id;
        queryOpts.app_code = inputOpts.app_code;
        delete queryOpts.format;
        if(typeof input === "string" || input instanceof String) {
          ...
          requestOpts.urlBuilder.host = "reverse.geocoder.cit.api.here.com:443";
          requestOpts.urlBuilder.pathname = "/6.2/reversegeocode.xml";
        } else {
          var err = new Error('You didn\t provide a valid input. Valid inputs are string or array')
          callback(err);
        }       
